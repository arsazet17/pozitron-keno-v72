...
(() => {
  ...
  function regimeBars(pred) {
    return pred.order.map(state => {
      ...
      const columnsText = available.length
        ? `${available.length} ст. (${available.map(col => `ст${col}`).join(', ')})`
        : 'нет';
      return `<div class="sm-reg${impossible ? ' sm-off' : ''}">
        <b>${stateLabel(state)}</b>
        <span>${impossible ? '0%' : pct(pred.probs[state] || 0)}</span>
        <small>${columnsText}</small>
      </div>`;
    }).join('');
  }
  ...
  function saveColumnForecast(model) {
    const last = draws.at(-1);
    if (!last || !model?.cols?.length) return;
    const rows = loadColumnForecasts();
    const type = model.name === 'СПРИНТ' ? 'sprint' : 'marathon';
    const exists = rows.some(row => row.type === type && Number(row.afterDraw) === Number(last.draw));
    if (exists) return; // не переписываем прогноз задним числом

    rows.push({
      type,
      afterDraw: Number(last.draw),
      createdAt: new Date().toISOString(),
      columns: model.cols.slice(0, 4).map(item => Number(item.col))
    });

    const trimmed = rows.slice(-800);
    try { localStorage.setItem(FORECAST_STORAGE_KEY, JSON.stringify(trimmed)); }
    catch (_) { /* localStorage может быть недоступен */ }
  }

  function forecastMark(type, previousDrawNumber, actualWinner) {
    const row = loadColumnForecasts().find(item =>
      item.type === type && Number(item.afterDraw) === Number(previousDrawNumber)
    );
    if (!row || !Array.isArray(row.columns)) return '';
    const place = row.columns.indexOf(Number(actualWinner));
    if (place === 0) return '<span class="sm-hit sm-hit-first" title="Первый прогноз">✓</span>';
    if (place > 0 && place < 4) return '<span class="sm-hit sm-hit-other" title="Попадание в прогноз 2–4">✓</span>';
    return '';
  }

  function drawBlockHtml(startIndex, endIndex, modelType) {
    ...
    for (let i = startIndex; i <= endIndex; i += 1) {
      ...
      const previousDraw = i > 0 ? draws[i - 1]?.draw : null;
      const mark = previousDraw == null ? '' : forecastMark(modelType, previousDraw, winner);
  ...
  function chainBlocksHtml(model) {
    const blocks = [];
    const modelType = model.name === 'СПРИНТ' ? 'sprint' : 'marathon';
    for (let start = model.startIndex; start <= model.endIndex; start += 5) {
      blocks.push(drawBlockHtml(start, Math.min(model.endIndex, start + 4), modelType));
    }
    return blocks.join('');
  }
  ...
  function render(which) {
    ...
    setTimeout(() => {
      ...
      saveColumnForecast(sprint);
      saveColumnForecast(marathon);
  ...
  function styles() {
    ...
    style.textContent = `
      ...
      .sm-chain-block .sm-hit{display:inline-block;margin-left:2px;font-size:.9em;font-weight:950}.sm-chain-block .sm-hit-first{color:#62e58b}.sm-chain-block .sm-hit-other{color:#66bfff}
