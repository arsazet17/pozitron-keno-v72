...
(() => {
  ...
  function sprintModel(dayIndices) {
    const chosen = dayIndices.slice(-5);
  ...
  function render(which) {
    ...
    setTimeout(() => {
      ...
      box.innerHTML = selector + `<div class="small sm-day-count">За ${selectedDate}: ${day.length} тиражей. Спринт — последние ${Math.min(5, day.length)}, Марафон — ${Math.min(40, day.length)}.</div>` + content;
