...
(() => {
  ...
  const fmtDate = d => d?.date || '';
  const fmtTime = d => d?.time || '';
  ...
  function saveForecastHistory(items) {
    try { localStorage.setItem(HISTORY_KEY, JSON.stringify(items.slice(-30))); }
    catch (_) { /* localStorage может быть недоступен */ }
  }

  function rememberForecast(baseDraw, forecast, windowSize, limit) {
    const history = loadForecastHistory();
    const key = String(baseDraw);
    const item = {
      baseDraw: Number(baseDraw),
      targetDraw: Number(baseDraw) + 1,
      numbers: forecast.numbers.slice(0, 12),
      ranges: forecast.ranges.slice(0, 6),
      windowSize,
      limit,
      savedAt: new Date().toISOString()
    };
    const index = history.findIndex(x => String(x.baseDraw) === key);
  ...
  function verificationHtml() {
    ...
    history.forEach(saved => {
      ...
      if (baseIndex < 0 || actualIndex < 0) return;

      const baseBalls = new Set(draws[baseIndex].balls || []);
      const actualBalls = new Set(draws[actualIndex].balls || []);
      const actualTransitions = [...actualBalls].filter(n => baseBalls.has(n));
      const hits = (saved.numbers || []).filter(n => actualTransitions.includes(n));
      const firstHit = hits.includes(saved.numbers?.[0]);
      const topFourHits = (saved.numbers || []).slice(0, 4).filter(n => actualTransitions.includes(n));

      const beforeOcc = occupancy(baseIndex);
      const afterOcc = occupancy(actualIndex);
      const actualUpRanges = afterOcc.map((v, r) => v > beforeOcc[r] ? r : -1).filter(r => r >= 0);
      const rangeHits = (saved.ranges || []).filter(r => actualUpRanges.includes(r));

      checks.push({ saved, baseIndex, actualIndex, actualTransitions, hits, firstHit, topFourHits, rangeHits });
    });

    if (!checks.length) return '';
    const latest = checks.sort((a, b) => b.saved.baseDraw - a.saved.baseDraw)[0];
    const predicted = latest.saved.numbers.map((n, i) => {
      const hit = latest.hits.includes(n);
      const mark = hit ? (i === 0 ? '✅' : i < 4 ? '☑️' : '•') : '';
      return `${pad2(n)}${mark}`;
    }).join(' ');
    const fact = latest.actualTransitions.length ? latest.actualTransitions.map(pad2).join(' ') : 'нет';
    const rangeFact = latest.rangeHits.length
      ? latest.rangeHits.map(rangeLabel).join(' ')
      : 'нет';

    return `<div class="section"><span>Проверка прошлого прогноза</span></div>
      <div class="row tr-check">
        <b>№${latest.saved.baseDraw} → №${latest.saved.targetDraw}</b>
        <div class="small">Прогноз: ${predicted}</div>
        <div class="tr-check-fact">Факт переходов: ${fact}</div>
        <div class="tr-check-total">Совпало ${latest.hits.length}/${latest.saved.numbers.length} · в первых 4: ${latest.topFourHits.length} · диапазоны ${latest.rangeHits.length}/${latest.saved.ranges.length}</div>
        <div class="small">Подтвердились диапазоны: ${rangeFact}</div>
      </div>`;
  }
  ...
  function compare(target, candidate) {
    ...
    const score =
      currentTransitions * 0.22 +
      occupancyScore * 0.18 +
      transitionRhythm * 0.15 +
      chains * 0.12 +
      transitionColumns * 0.10 +
      rangeFlow * 0.09 +
      movement * 0.06 +
      pattern * 0.04 +
      alternation * 0.02 +
    ...
    return { score, transitionRhythm, currentTransitions, transitionColumns, rangeFlow, occupancyScore, movement, pattern, chains, alternation, shape };
  }
  ...
  function buildForecast(rows, targetIndex) {
    ...
    const currentOcc = occupancy(targetIndex);
    ...
    rows.forEach((row, rank) => {
      ...
      const beforeOcc = occupancy(row.index);
      const afterOcc = occupancy(row.nextIndex);
      ...
      nextTransitions.forEach(n => {
        ...
        numberCases[n] += 1;
        rangeScores[rangeOf(n)] += weight * 0.65;
      });

      for (let r = 0; r < 20; r += 1) {
        const delta = afterOcc[r] - beforeOcc[r];
        const retained = Math.min(beforeOcc[r], afterOcc[r]);
        const targetDensity = currentOcc[r] / 4;
        rangeScores[r] += weight * (
          Math.max(0, delta) * 0.95 +
          retained * 0.18 +
          targetDensity * 0.12
        );
      }
  ...
  function render() {
    ...
    const forecast = buildForecast(top, targetIndex);
    const checkHtml = verificationHtml();
    rememberForecast(draws[targetIndex].draw, forecast, windowSize, limit);
    ...
    const analogs = top.slice(0, 12).map((row, rank) => {
      ...
      return `<div class="tr-analog">
        <div class="tr-analog-head"><b>${rank + 1}. №${old.draw} • ${fmtDate(old)} • ${fmtTime(old)} 🔴 ст${(old.column||columnOf((oldPass[0]||10)))}</b><strong>${pct(row.score)}</strong></div>
        <div class="small">Переходы тогда: ${oldPass.length ? oldPass.map(pad2).join(' ') : 'нет'}</div>
        <div class="tr-next">➡ №${next.draw}: ${nextPass.length ? nextPass.map(pad2).join(' ') : 'без переходов'} 🔴 ст${(next.column||columnOf((nextPass[0]||10)))}</div>
        <div class="breakdown">
          <span>переходы ${pct(row.currentTransitions)}</span><span>заполняемость ${pct(row.occupancyScore)}</span>
          <span>ритм ${pct(row.transitionRhythm)}</span><span>диапазоны ${pct(row.rangeFlow)}</span>
          <span>цепочки ${pct(row.chains)}</span><span>столбы ${pct(row.transitionColumns)}</span>
        </div>
      </div>`;
    }).join('');
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .tr-check{border-color:#496987}.tr-check-fact{margin-top:7px;font-weight:900;color:#ffd35c}.tr-check-total{margin-top:7px;font-weight:950;color:#8eedaa}
